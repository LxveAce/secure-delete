...
- **Optional TPM root (`init --tpm`) closes that residual in hardware.** The master is wrapped under `HKDF(RWK ‖ KEK)` where RWK is sealed by the TPM; `hardware-shred` destroys the TPM key so a stale wrapped-master in flash can't be opened even with the passphrase. **Honest limit:** on Windows (Platform Crypto Provider) the TPM key is an SRK-wrapped `.PCPKEY` blob on disk, usable only on *this* TPM — deleting it is defense-in-depth (a remote flash-image + passphrase attacker is defeated), **not** an absolute in-chip erase. A tampered header can't downgrade a TPM vault to software (the master seal binds the root descriptor as AEAD AAD). A **mandatory one-time recovery code** (unless `--i-understand-total-loss`) survives TPM loss; guard it like the data (code + vault folder + passphrase reopens it).
